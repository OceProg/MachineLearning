...
object tp_reg1 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    //    var res_sd = sqrt((res.map { x => (x-res_mean)*(x-res_mean) }.reduce(_+_))/res.size)
    //    var res_what = (res.map { x => abs(x-res_mean)}.reduce(_+_))/res.size
    //    res.foreach(println)
    //    println("res_mean = "+res_mean)
    //    println("res_sd = "+res_sd)
    //    println("res_what = "+res_what)
